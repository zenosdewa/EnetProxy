...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
        ...
        if (find_command(chat, "name ")) { //ghetto solution, but too lazy to make a framework for commands.
            ...
            g_server->send(false, "action|dialog_return\ndialog_name|drop_item\nitemID|242|\ncount|" + cdropcounts); // 242 is id of World Lock
            gt::send_log("`9Dropped `c" + cdropcounts + "`9 bgl");
            return true;
        }
            ...
            g_server->send(false, "action|dialog_return\ndialog_name|drop_item\nitemID|1796|\ncount|" + cdropcounts); // 1796 is id of Diamond Lock
            gt::send_log("`9Dropped `c" + cdropcounts + "`9 dl");
            return true;
        }
        ...
        g_server->send(false, "action|dialog_return\ndialog_name|drop_item\nitemID|7188|\ncount|" + cdropcounts); // 7188 is id of Blue Gem Lock
        gt::send_log("`9Dropped `c" + cdropcounts + "`9 bgl");
        return true;
        }
        ...
        paket =
            ...
            "\nadd_spacer|small"
            "\nadd_textbox|`1/proxy `#(Show Proxy Commands)|left|2480|"
            "\nadd_textbox|`2/dropwl [amount] `#(Drop World Lock by amount)|left|2480|"
            "\nadd_textbox|`3/dropdl [amount] `#(Drop Diamond Lock by amount)|left|2480|"
            "\nadd_textbox|`6/dropbgl `#(Drop Blue Gem Lock by amount)|left|2480|"
            "\nadd_textbox|`6/wrenchset [pull/kick/ban] `#(Change wrench mode)|left|2480|"
            "\nadd_textbox|`6/wm `#(Enable/Disable wrench mode)|left|2480|"
            ...
            "\nadd_textbox|`9/fd `#(Enable/Disable Fast Drop)|left|2480|"
            "\nadd_textbox|`9/ft `#(Enable/Disable Fast Trash)|left|2480|"
